...
proc toRunes*(self: UnicodeVariant): seq[Rune] =
  if self.ascii: self.asciiStr.toRunes
  else: self.unicodeStr

proc asUnicode*(self: UnicodeVariant): UnicodeVariant =
  ## Convert to `UnicodeVariant` object.
  if self.ascii:
    newUnicodeUnicodeVariant(self.asciiStr.toRunes)
  else:
    self
