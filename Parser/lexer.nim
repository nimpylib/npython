...
proc lexFStringInExpr(lexer: Lexer, line: string, idx: var int, the_current_tok: tokenizer_mode, mode: Mode): TokenNode =
  template tryCloseExpr =
      ...
      if INSIDE_FSTRING_EXPR(lexer.getMode()):
        let nFileName = lexer.fileName & "-line"
        var nlexer = newLexer(nFileName, lexer.lineNo)
        let cur_tok = lexer.popMode()

        nlexer.lexOneLine line[cur_tok.start ..< idx], mode
        assert nlexer.tokenNodes[^1].token == Newline
        discard nlexer.tokenNodes.pop(); nlexer.lineNo.dec
        for i in nlexer.tokenNodes: lexer.add i
